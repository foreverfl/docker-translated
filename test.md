...
sidebar_label: "docker container run"
slug: "/reference/cli/docker/docker-container-run"
---
...
| `--annotation` | `map[]` | Add an annotation to the container (passed through to the OCI runtime)<br /> |
| `-a --attach` |  | Attach to STDIN, STDOUT or STDERR |
| `--blkio-weight` | `0` | Block IO (relative weight), between 10 and 1000, or 0 to disable (default 0)<br /> |
| `--blkio-weight-device` | `[]` | Block IO weight (relative device weight) |
| `--cap-add` |  | Add Linux capabilities |
| `--cap-drop` |  | Drop Linux capabilities |
| `--cgroup-parent` |  | Optional parent cgroup for the container |
| `--cgroupns` |  | Cgroup namespace to use (host|private)<br />'host':    Run the container in the Docker host's cgroup namespace<br />'private': Run the container in its own private cgroup namespace<br />'':        Use the cgroup namespace as configured by the<br />           default-cgroupns-mode option on the daemon (default) |
...
| `--health-start-interval` | `0s` | Time between running the check during the start period (ms|s|m|h) (default 0s)<br /> |
| `--health-start-period` | `0s` | Start period for the container to initialize before starting health-retries countdown (ms|s|m|h) (default 0s)<br /> |
| `--health-timeout` | `0s` | Maximum time to allow one check to run (ms|s|m|h) (default 0s) |
| `--help` |  | Print usage |
| `-h --hostname` |  | Container host name |
| `--init` |  | Run an init inside the container that forwards signals and reaps processes<br /> |
...
| `--memory-swap` | `0` | Swap limit equal to memory plus swap: '-1' to enable unlimited swap<br /> |
